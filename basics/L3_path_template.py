...
motions = [
    [0.25, 0.0, 2.0],            # Motion 1
    [0.0, 0.8, 2.0],            # Motion 2
    [0.25, 0.0, 2.0],
    [0.0, 0.8, 2.0],
    [0.25, 0.0, 2.0],
    [0.0, -0.8, 2.0],
...
for  count, motion in enumerate(motions):
    ...
    log.tmpFile(motion[0], "velocity2.txt")
    log.tmpFile(motion[1], "angular.txt")
    wheel_speeds = ik.getPdTargets(motion[:2])                  # take the forward speed(m/s) and turing speed(rad/s) and use inverse kinematics to deterimine wheel speeds
